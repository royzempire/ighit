...
def info(username,jj):
  ...
  try:
    ...
    zaid=requests.post("http://www.bradychrist.top/api/v7/user",headers=he,data={"username": username}).json()
    print(zaid)
    id=zaid['data']['userPk']
    follower_count=zaid['data']['followerNum']
    following_count=zaid['data']['followingNum']
    media_count=zaid['data']['postsNum']
    try:
      date1=str(date(id))
    except:
      date1='None'
...
def check(email):
  ...
  try:
   ...
   #   print(str(response))
      if 'showAccountRecoveryModal' in response:Qredes(email)
      else:bads_instgram+=1
  except:''
   # check(email)
  os.system('clear' if os.name == 'posix' else 'cls')
